...
import (
	...
	"github.com/stockyard-dev/stockyard-broadside/internal/store"
)

// resourceName is the canonical key for extras storage and the API path.
const resourceName = "press_releases"
...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{
			"tier":        s.limits.Tier,
			"upgrade_url": "https://stockyard.dev/broadside/",
		})
	})
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("broadside: warning: could not parse config.json: %v", err)
		return
	}
	s.pCfg = cfg
	log.Printf("broadside: loaded personalization from %s", path)
}
...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"press_releases": oe(s.db.Search(q, filters))})
		return
	}
...
func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	if s.limits.MaxItems > 0 && s.db.Count() >= s.limits.MaxItems {
		we(w, 402, "Free tier limit reached. Upgrade at https://stockyard.dev/broadside/")
		return
	}
	var e store.PressRelease
